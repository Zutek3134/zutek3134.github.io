...
- 為所有關卡重新進行編碼。
- 新增收集物：隨機傳送。
- 成績結算畫面為皇冠增加標題並且改變計算方法，詳洽[關於](/Zutek3134/zh-TW/games/STF/about)。
- 修復前次更新加入收集物所導致的錯誤。
...
- 新增「二段跳」。
- 新增兩個預設設定：奪旌搶旗、極速通關。
- `乙類關卡` 中低機率生成「子彈」（修改自 `L9：Boss 關卡１`）。
- 更新「子彈」跟隨機制。
- 更新「Coin Master 模式」的分數計算規則，詳洽[關於](/Zutek3134/zh-TW/games/STF/about)。
- 隱藏 `Boss 關卡２`。
- 取消隱藏 `L11：水關卡３`。
...
- 改變時間計算規則，令 `新時間 = 目前時間 + 時間球影響秒數`，分為三種情況：
    ...
    - 禁止加秒：上述皆不成立時。
    - 意即若「已經過遊戲時長」或「新時間」已超過「關卡長度的兩倍」，則吃時間球無效。
- 優化玩家碰撞箱。
